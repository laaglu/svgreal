		...
		QUADRATIC {
			...
			boolean processMouseDown(AddPathCommandFactory factory, OMSVGPoint p, float hs) {
				...
				if (factory.ctrlPressed) {
					...
					if (prevSegRep != null) {
						prevSegRep.setCp2(factory.svg.createSVGPoint(2 * segRep.getX() - p.getX(), 2 * segRep.getY() -p.getY()), hs);
					}
				}
				return true;
			}
				...
				if (segRep != null) {
					if (factory.mousePressed) {
						...
						if (factory.ctrlPressed) {
							SVGSegRep prevSegRep = factory.previousCurve();
							if (prevSegRep != null) {
								prevSegRep.processMouseMove(factory.svg.createSVGPoint(-delta.getX(), -delta.getY()), prevSegRep.getCp2(), hs, false);
							}
						}
					} else {
						// Update the endpoint
...
//		GWT.log("AddPathCommandFactory.processMouseDown(" + state + ")");
		...
		boolean processed = state.processMouseDown(this, getCoordinates(event, m), SVGModel.getVertexSize(path));
		if (processed) {
...
//		GWT.log("AddPathCommandFactory.processMouseUp(" + state + ")");
		boolean processed = state.processMouseUp(this, getCoordinates(event, m), SVGModel.getVertexSize(path));
		if (processed) {
			event.stopPropagation();
			event.preventDefault();
		}
