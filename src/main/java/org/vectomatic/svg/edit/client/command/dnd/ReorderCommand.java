...
public class ReorderCommand extends DndCommandBase {
	...
	public ReorderCommand(List<SVGElementModel> models, SVGElementModel target, DropGesture dropGesture) {
		...
		for (SVGElementModel model : sortedModels) {
			...
			if (this.owner == null) {
				...
				assert this.owner == owner : "Attempt to move models from heterogeneous owners: " + this.owner + ", " + owner;
			}
	...
	public BeanModel asModel() {
		return BeanModelLookup.get().getFactory(ReorderCommand.class).createModel(this);
	}
