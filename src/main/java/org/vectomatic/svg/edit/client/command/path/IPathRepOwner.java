...
public interface IPathRepOwner {
	...
	/**
	 * Enum to determine if a path is open or closed
	 */
	public enum Closure {
		OPEN {
			@Override
			public void write(OMSVGPathElement path) {
				path.removeAttribute(CLOSED_ATTRIBUTE);
			}
		...
		CLOSE {
			@Override
			public void write(OMSVGPathElement path) {
				path.setAttribute(CLOSED_ATTRIBUTE, "true");
			}
		};
		public abstract void write(OMSVGPathElement path);
		static final String CLOSED_ATTRIBUTE = "closed";
	}
