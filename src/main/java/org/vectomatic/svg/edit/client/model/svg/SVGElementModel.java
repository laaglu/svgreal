...
import org.vectomatic.svg.edit.client.model.IValidatable;
 ...
 * @author laaglu
 */
public abstract class SVGElementModel extends AbstractModel<SVGElement> implements TreeModel, IValidatable<SVGElement> {
...
//		GWT.log("set(" + property + ") = " + oldValue + ", " + value);
		...
		ValidationError error = propertyDefinition.validate(element, value);
		((IValidatable)this).updateSeverity(error == null ? null : error.getSeverity());
		return oldValue;
	}
	 ...
	 * @param severity The severity of a child of this model 
	 * or a property of this model
	 * @return
	 */
	public void updateSeverity(Severity severity) {
		// Present severity | childSeverity | New severity
		// Valid			| Valid		    | unchanged
		// Valid			| Warning	    | Warning + update parent
		// Valid			| Error		    | Error + update parent
		// Warning			| Valid		    | Recompute
		// Warning			| Warning	    | unchanged
		// Warning			| Error		    | Error + update parent
		// Error			| Valid		    | Recompute
		// Error			| Warning	    | Recompute
		// Error			| Error		    | unchanged
		
		if (this.severity == severity) {
			return;
		}
		if ((this.severity == null && severity != null)
		||  (this.severity == Severity.WARNING && severity == Severity.ERROR)) {
			this.severity = severity;
			if (parent != null) {
				((IValidatable)parent).updateSeverity(severity);
			}
		} else {
	 ...
	 * @return the severity of this model
	 */
	public Severity computeSeverity() {
		severity = null;
		MetaModel<SVGElement> metaModel = getMetaModel();
		for (String propertyName : metaModel.getPropertyNames()) {
			IMetadata propertyDefinition = metaModel.getMetadata(propertyName);
			ValidationError error = propertyDefinition.validate(element, propertyDefinition.get(element));
			if (error != null) {
		...
		if (severity != Severity.ERROR) {
			for (ModelData child : children) {
				Severity childSeverity = ((SVGElementModel)child).computeSeverity();
...
//		GWT.log(get(SVGConstants.SVG_TITLE_TAG) + " = " + severity);
		SvgrealApp.getApp().getWindow(element).updateIcon(this);
		return severity;
	}
	
	@Override
	public Set<ValidationError> getErrors(SVGElement model, Severity severity) {
		...
		for (String propertyName : metaModel.getPropertyNames()) {
			...
			ValidationError error = propertyDefinition.validate(element, propertyDefinition.get(element));
			if (error != null) {
				if (severity == null || error.getSeverity() == severity) {
					errors.add(error);
				}
			}
		}
