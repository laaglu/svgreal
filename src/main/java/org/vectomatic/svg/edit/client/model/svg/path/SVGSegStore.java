...
public class SVGSegStore extends ListStore<SVGSegModel> {
	...
	/**
	 * True if this path is closed, false otherwise
	 */
	...
	public void update(OMSVGPathElement path) {
		GWT.log("SVGSegStore.update");
		// Clear selection
		Set<SVGSegModel> oldSelection = new HashSet<SVGSegModel>(selectionModel.getSelectedItems());
		...
		for (int i = 0, size = getCount(); i < size; i++) {
			SVGSegModel seg = getAt(i);
			if (oldSelection.contains(seg)) {
				selectionModel.select(seg, true);
			}
		}
	...
	public void setClosed(boolean closed) {
		if (this.closed != closed) {
			...
			if (closed) {
				...
				segs.removeItem(segs.getNumberOfItems() - 1);
			}
			this.closed = closed;
	...
	public String setPathSegType(SVGSegModel segModel, String type) {
		...
		if (!oldType.equals(type)) {
			...
			float x = segModel.get(SVGConstants.SVG_X_ATTRIBUTE);
			float y = segModel.get(SVGConstants.SVG_Y_ATTRIBUTE);
			OMSVGPathElement path = (OMSVGPathElement) model.getElementWrapper();
			OMSVGPathSegList segs = path.getPathSegList();
			switch (SVGPathSegType.INSTANCE.fromName(type)) {
			    ...
			    case OMSVGPathSeg.PATHSEG_CURVETO_CUBIC_ABS:
				  	{
						float x0 = prevSegModel.get(SVGConstants.SVG_X_ATTRIBUTE);
						float y0 = prevSegModel.get(SVGConstants.SVG_Y_ATTRIBUTE);
